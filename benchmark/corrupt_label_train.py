...
def create_save_dir():
    if opt.add_defense:
        return 'checkpoints/label/MMD_defense_{}_{}_{}_data_{}_arch_{}_mode_{}_auglist_{}_rlabel_{}'.format(opt.defense[0],opt.defense[1],opt.noise_position,opt.data, opt.arch, opt.mode, opt.aug_list, opt.rlabel)
    if opt.MoEx and opt.Mixup:
        return 'checkpoints/label/MixupMoex_{}_data_{}_arch_{}_mode_{}_auglist_{}_rlabel_{}'.format(opt.alpha,opt.data, opt.arch, opt.mode, opt.aug_list, opt.rlabel)
    elif opt.MoEx:
        return 'checkpoints/label/MoEx_data_{}_arch_{}_mode_{}_auglist_{}_rlabel_{}'.format(opt.data, opt.arch, opt.mode, opt.aug_list, opt.rlabel)
         ...
         return 'checkpoints/label/Mixup_{}_data_{}_arch_{}_mode_{}_auglist_{}_rlabel_{}'.format(opt.alpha,opt.data, opt.arch, opt.mode, opt.aug_list, opt.rlabel)
    else:
