...
def create_save_dir():
    # if opt.add_defense:
    #     return 'benchmark_copy/images/MMD_defense_{}_{}_{}_data_{}_arch_{}_epoch_{}_optim_{}_mode_{}_auglist_{}_rlabel_{}'.format(opt.defense[0],opt.defense[1],opt.noise_position, opt.data, opt.arch, opt.epochs, opt.optim, opt.mode, \
    #         opt.aug_list, opt.rlabel)

    # if opt.MoEx and opt.Mixup:
    #     return 'benchmark_copy/images/MixupMoex_data_{}_arch_{}_epoch_{}_optim_{}_mode_{}_auglist_{}_rlabel_{}'.format(opt.data, opt.arch, opt.epochs, opt.optim, opt.mode, \
    #     opt.aug_list, opt.rlabel)
    # elif opt.MoEx:
    #     return 'benchmark_copy/images/MoEx_data_{}_arch_{}_epoch_{}_optim_{}_mode_{}_auglist_{}_rlabel_{}'.format(opt.data, opt.arch, opt.epochs, opt.optim, opt.mode, \
    #     opt.aug_list, opt.rlabel)
    # elif opt.Mixup:
    #     return 'benchmark_copy/images/Mixup_data_{}_arch_{}_epoch_{}_optim_{}_mode_{}_auglist_{}_rlabel_{}'.format(opt.data, opt.arch, opt.epochs, opt.optim, opt.mode, \
    #     opt.aug_list, opt.rlabel)
    # else:
...
def reconstruct(idx, model, loss_fn, trainloader, validloader):
    ...
    if opt.MoEx and opt.Mixup:
        method = 'MixupMoEx_'+str(opt.alpha)
    elif opt.MoEx:
        method = 'MoEx'
    elif opt.Mixup:
        method = 'Mixup_'+str(opt.alpha)
    else:
        method = ''
...
def create_checkpoint_dir():
    ...
    return 'checkpoints/Mixup/Mixup_lambda_{}_data_{}_arch_{}_mode_{}_auglist_{}_rlabel_{}'.format(opt.alpha,opt.data, opt.arch, opt.mode, opt.aug_list, opt.rlabel)
